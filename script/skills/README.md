...
- `wechat-play/SKILL.md`：绑在这台微信登录态上的玩法（随机朋友等）；不写进 mcp-tools
...
框架通道记录是 `agbot_chat_history` / `agbot_chat_get`，随机朋友是 `golem_random_friend`（boot `/mcp`），和 cf-mcp-tools 不是同一个 MCP。
