...
- `text`=按下表转好的指定指令，不要把用户原话塞进去（带「庄园」的口语也不行，例如「庄园帮我浇一下」要转成 `庄园浇水`）
- 吃不准：`text`=`庄园帮助`，或 `listHelp=true`
- 工具回没听懂：按意图再转一次指定指令，仍不成再用人话问一句

| 意图 | `text` 转成 |
|---|---|
| 看院子 / 状态 / 一览 | `庄园` |
| 院子长啥样 | `庄园主院` |
| 收菜 / 收作物 / 收获 / 把能收的都收了 | `庄园收获` |
| 打理 / 浇喂捉 / 日常 | `庄园打理` |
| 补种 / 再种上 | `庄园补种` 或 `庄园补种 小麦` |
| 卖掉 / 出售 | `庄园出售 全部`；按品质 `庄园出售 普通`；按种类 `庄园出售 作物` / `畜产` / `鱼` / `小麦`；点名 `庄园出售 普通小麦 2` |
| 浇水 | `庄园浇水` |
| 除虫 / 捉虫 | `庄园捉虫` |
| 种某作物 | `庄园种植 作物名` |
| 升门楼 / 正房 / 厢房 / 凉亭 / 仓房 | `庄园升级 建筑名` |
| 钓鱼 | `庄园钓鱼` |

主院描写是写死的，原样转给用户，不要润色、不要补金币等级。建筑升不上去时按工具回的限制说。对方明确要画院子：先 `庄园主院`，返回原文当 `draw_image` 的 prompt。闲聊不调。
