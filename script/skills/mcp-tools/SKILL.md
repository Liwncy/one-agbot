...
- 对方要画自己的院子：先 `manor_action` `text=庄园主院`，把返回原文当 `prompt`，不要改写。只看主院、升级、一览时不要自己出图
...
种地、喂养、钓鱼、主院、商店等带「庄园」的口令 → `manor_action`（不要走修仙）：

- `platform`=`agbot`（必填）
- `userId`=发言人 wxid（院子绑定键；不要念给用户听，也不要用 wxid 当院名）
- `userName`=昵称（不要填成 wxid）；没院子时用它当默认院名
- `text`=带「庄园」的原话（如「庄园浇水」「庄园主院」「庄园升级 门楼」）
- 只要目录可 `listHelp=true`

主院描写是写死的，原样转给用户，不要润色、不要补金币等级。建筑升不上去时按工具回的庄园等级限制说，不要改描写。对方明确要画院子时，用「庄园主院」返回原文当 `draw_image` 的 prompt。
