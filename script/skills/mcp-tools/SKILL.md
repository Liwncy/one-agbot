---
name: MCP 工具用法
description: 小聪明儿调用 MCP 的分流与参数。通道聊天记录走 agbot_*；热点时事走 trend_*；其余走 cf-mcp-tools。普通闲聊不调工具。
---
...
- 闲聊、接话、问好：不调工具。明确问热搜/热榜/今天有啥瓜走「热点与时事」；普通闲聊只有提到具体时事或热梗、又吃不准背景时才可 `trend_search`，不要为了硬接梗每条都查
...
热点约每 30 分钟更新。问「现在 / 今天 / 最近」时必须先查，不要拿模型记忆冒充实时结果。

- 看整体热榜、今天有啥瓜 → `trend_digest`
  - `platforms` 可选：`zhihu` / `baidu` / `bili` / `toutiao` / `douyin`；对方没指定就不填
  - `limitPerPlatform` 默认 10；聊天只挑最值得说的 1～3 条，不要逐条念榜
...
- 多个平台标题在说同一件事时合并讲，不要重复报。热榜只说明「正在被讨论」，不等于事实已证实；优先按返回的 `detail` 说，只有标题时别自己补内幕
- 回答像朋友聊两句：先说最有意思或最相关的，再补「在哪个平台看到、多久前更新」。不要输出 JSON、字段名或大段链接；对方要来源时再给 `url`
- 普通聊天只在已经聊到具体时事/梗且确实需要背景时查 `trend_search`；不要主动开启新话题，不要每轮刷新
