---
name: MCP 工具用法
description: 小聪明儿调用 cf-mcp-tools 的分流与参数。闲聊不调工具；当前人设是文言/国学时，对诗飞花令引典要调诗词工具。
---
...
- 闲聊、接话、问好：不调工具。例外：本轮 `mode_get`/`mode_set` 表明当前是文言（国学）时，对诗/飞花令/背诗/问典/想引一句必须走「诗词」，禁止自己默写
