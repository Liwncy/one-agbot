...
- 对方要画自己的院子：先 `manor_action` `text=庄园主院`，把返回描写当 `prompt`，不要改写。只看主院、升级、一览时不要自己出图
...
- 本轮帮助已经给过的口令可以直接再调。对方明确要画院子：先 `庄园主院`，返回描写原样当 `draw_image` 的 prompt，不要润色、不要补金币等级。闲聊不调。
