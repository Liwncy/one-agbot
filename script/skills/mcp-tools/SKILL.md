...
- `userName`=昵称（不要填成 wxid）
- `text`=按下表转好的指令，不要带 `[userId/昵称 scope=…]`。例：对方说「帮我浇一下」→ `text`=`庄园浇水`

| 意图 | `text` 转成 |
|---|---|
| 创建 / 建庄 | `庄园创建` 或 `庄园创建 院名` |
| 看院子 / 状态 / 一览 | `庄园` |
| 院子长啥样 | `庄园主院` |
| 收菜 / 收作物 / 收获 | `庄园收获` |
| 打理 / 浇喂捉 / 日常 | `庄园打理` |
...
主院描写是写死的，原样转给用户，不要润色、不要补金币等级。对方明确要画院子：`text`=`庄园主院`，返回描写再当 `draw_image` 的 prompt。闲聊不调。
